...
本功能负责扫描目标项目目录，识别和加载源代码文件，为后续的架构分析和注释生成提供原始数据输入。
...
系统 SHALL 递归扫描用户指定的目标项目根目录，发现所有子目录和文件，并支持通过命令行参数指定项目路径。

#### Scenario: 正常扫描完整项目

- **WHEN** 用户通过命令行指定一个有效的项目根目录路径
- **THEN** 系统递归遍历该目录下所有子目录和文件
- **AND** 返回完整的 SourceFile 对象列表，按相对路径排序
