...
3. **所有函数/方法 MUST 添加函数级注释**：无论函数体代码简单还是复杂、无论行数多少，都 MUST 在函数/方法定义上方添加函数级中文注释，说明函数的作用和业务目的。当函数体代码在 5 行以内（不含函数定义行和空行）时，不对函数体内部逐行注释，但函数级注释不可省略。
...
6. **禁止重复添加中文注释**：对于代码行或函数体上方已经存在中文注释的，MUST 保留已有注释不做修改，不得重复生成新的中文注释。仅对尚无中文注释的代码段落添加新注释。
7. **禁止简单翻译变量名/函数名/类名作为注释**：注释 MUST NOT 只是把变量名、函数名、类名翻译成中文。注释 MUST 说清该段代码或函数的实际作用、业务目的或在整体流程中的角色。例如 `getUserName()` 不应注释为"获取用户名"，而应说明从哪里获取、用于什么业务场景；`orderList` 不应注释为"订单列表"，而应说明该列表在当前上下文中承担的角色和数据来源。
8. **所有类定义 MUST 添加类级注释**：无论类是简单（字段少、方法少）还是复杂，都 MUST 在类定义上方添加类级中文注释，描述类的设计目的、核心职责和在系统中的角色，不得跳过任何类。
...
3. **避免冗余注释**：import/from 导入语句、简单变量声明、简单逻辑计算不生成注释；5 行以内的简短函数须生成函数级注释但不逐行注释。所有函数和类均须生成对应级别的注释。
